...
const game = () => {
    ...
    // --- Oyun Değişkenleri ---
    let playerData = {}; // Oyuncu bilgilerini saklamak için
    ...
    // YENİ: Nişan alma hedefinin koordinatlarını saklamak için
    const aimTarget = {
        x: 0,
        y: 0,
        visible: false // Başlangıçta imleç görünmez
    };
    ...
    // YENİ: Nişan alma imlecini çizen fonksiyon
    const drawAim = () => {
        if (!aimTarget.visible) return; // Eğer görünür değilse çizme
        ctx.strokeStyle = 'rgba(255, 255, 255, 0.7)';
        ctx.lineWidth = 2;
        ctx.beginPath();
        ctx.arc(aimTarget.x, aimTarget.y, 15, 0, 2 * Math.PI); // Dış daire
        ctx.moveTo(aimTarget.x - 10, aimTarget.y);
        ctx.lineTo(aimTarget.x + 10, aimTarget.y); // Yatay çizgi
        ctx.moveTo(aimTarget.x, aimTarget.y - 10);
        ctx.lineTo(aimTarget.x, aimTarget.y + 10); // Dikey çizgi
        ctx.stroke();
    };
    ...
    const setupEventListeners = () => {
        const updateAimPosition = (e) => {
            ...
            // Hem fare hem dokunma olayları için koordinatları al
            const clientX = e.clientX || e.touches[0].clientX;
            const clientY = e.clientY || e.touches[0].clientY;
            aimTarget.x = clientX - rect.left;
            aimTarget.y = clientY - rect.top;
            aimTarget.visible = true; // İmleç artık görünür
        };

        // Fare hareketleri (Masaüstü)
        canvas.addEventListener('mousemove', updateAimPosition);

        // Dokunma hareketleri (Mobil)
        canvas.addEventListener('touchmove', (e) => {
            e.preventDefault(); // Sayfanın kaymasını engelle
            updateAimPosition(e);
        });

        // İmleç canvas dışına çıktığında hedefi gizle
    ...
    // --- Oyun Döngüsü ---
    // YENİ: Sürekli çalışan ana oyun döngüsü
    const gameLoop = () => {
        // 1. Ekranı Temizle
        ctx.clearRect(0, 0, canvas.width, canvas.height);

        // 2. Her şeyi Yeniden Çiz
        drawPitch();
        drawFieldMarkings();
        drawGoal();
        drawBall();
        drawAim(); // Nişan imlecini çiz
