...
const game = () => {
    ...
    const keeper = {
        ...
        diveTarget: { x: 0, y: 0 },
        diveSpeed: 0,
        idleAnimTime: 0 // bekleme animasyonu için zamanlayıcı
    };
    ...
    const drawAim = () => { /* ... kod aynı ... */ };

    // YENİ: Kaleciyi çizen fonksiyon
    const drawKeeper = () => {
        // Adidas temasına uygun renkler
        ctx.fillStyle = '#0052A2'; // Adidas mavisi
    ...
    const shoot = () => {
        ...
        // Şut çekildiğinde kaleci dalış kararını verir
        decideKeeperDive();
    };

    // YENİ: Kalecinin dalış kararını veren AI
    const decideKeeperDive = () => {
        // Topun kale çizgisine varacağı X pozisyonunu tahmin et
        const timeToGoal = (ball.y - goal.height) / ball.vy;
        const finalBallX = ball.x + ball.vx * timeToGoal;

        // Biraz rastgelelik ekle
        const errorMargin = goal.width * 0.3 * (Math.random() - 0.5); // Sağa veya sola sapma
        let targetX = finalBallX + errorMargin;

        // Hedefin kale direkleri içinde kaldığından emin ol
        targetX = Math.max(goal.x, Math.min(targetX, goal.x + goal.width - keeper.width));

        keeper.diveTarget.x = targetX;
        keeper.diveTarget.y = keeper.y; // Y ekseninde çok hareket etmez
        keeper.state = 'diving';
    };

    const updateKeeper = () => {
        if (keeper.state === 'idle') {
            // "Canlı" görünmesi için yavaşça kale çizgisinde hareket et
    ...
    const checkCollision = () => {
        ...
        // Kaleci ve top arasında basit bir çarpışma kontrolü
        const collides = ball.x > keeper.x && 
        ...
        if (collides) {
            ...
            gameState = 'result'; // Sonuç durumuna geç
            setTimeout(resetGame, 1500); // 1.5 saniye sonra oyunu sıfırla
        } else if (ball.y < goal.height) { // Top kale çizgisine ulaştıysa
    ...
    const updateBallPosition = () => {
        if (gameState === 'shooting') {
            ...
            ball.y += ball.vy;
            ball.radius *= 0.995;
            checkCollision(); // Çarpışma kontrolünü çağır
        }
    };
    
    const setupEventListeners = () => { /* ... kod aynı ... */ };
    ...
    const init = (pData) => {
        ...
        setupEventListeners();
        gameLoop();
        setTimeout(() => { gameReadyForInput = true; }, 100);
    };
    ...
    // Dummy definitions for drawing functions to avoid breaking the display
    // In a real scenario, these would contain the full drawing logic as provided before.
    const originalToString = game.toString();
