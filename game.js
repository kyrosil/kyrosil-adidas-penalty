...
const game = () => {
    ...
    // YENİ: Penaltı noktasını geriye çekmek için sabit bir oran tanımlıyoruz.
    const PENALTY_SPOT_Y_RATIO = 0.6; 

    // --- Seviye Zorluk Ayarları ---
    const levelConfig = {
        1: { keeperSpeed: 0.012, aiShotSpeed: 0.028 }, // AI şut hızını biraz artırdım
    ...
    const setupTurn = () => {
        const config = levelConfig[currentLevel]; message.visible = false;
        ball.radius = canvas.width * 0.025; ball.vx = 0; ball.vy = 0;
        ...
        if (gamePhase === 'player_shot') {
            ...
            gameState = 'save'; ball.x = canvas.width / 2;
            // GÜNCELLENDİ: Top, geriye çekilmiş penaltı noktasına konuluyor
            ball.y = canvas.height * PENALTY_SPOT_Y_RATIO; 
            ball.speed = canvas.width * config.aiShotSpeed; keeper.diveSpeed = canvas.width * 0.02;
    ...
    const drawFieldMarkings = () => {
        ...
        // GÜNCELLENDİ: Penaltı noktası yeni sabite göre çiziliyor
